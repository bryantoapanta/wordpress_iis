...
<head>
	...
	<style type="text/css">
		...
		table.blueTable thead {
			background: #00c0d0;
			background: -moz-linear-gradient(top, #00c0d0 0%, #00c0d0 66%, #00c0d0 100%);
			background: -webkit-linear-gradient(top, #00c0d0 0%, #00c0d0 66%, #00c0d0 100%);
			background: linear-gradient(to bottom, #00c0d0 0%, #00c0d0 66%, #00c0d0 100%);
		...
		table.blueTable tfoot {
			...
			color: #FFFFFF;
			background: #00c0d0;
			background: -moz-linear-gradient(top, #dcebf7 0%, #d4e6f6 66%, #00c0d0 100%);
			background: -webkit-linear-gradient(top, #dcebf7 0%, #d4e6f6 66%, #00c0d0 100%);
			background: linear-gradient(to bottom, #dcebf7 0%, #d4e6f6 66%, #00c0d0 100%);
...
<body>
	...
	//$serverName = "172.16.0.22"; //serverName\instanceName
	...
	$query = "SELECT
		TopRanking = ROW_NUMBER () OVER (	ORDER BY	COUNT (		CAST (	dbo.CUSTOMER.[IDENTITY] AS INT ) ) DESC),
		dbo.SHOP.[IDENTITY] AS idTienda,
		UPPER (dbo.SHOP.INTERNALNAME) AS NOMBRE,
		COUNT (	CAST (	dbo.CUSTOMER.[IDENTITY] AS INT	)	) AS TOTAL
		FROM
			dbo.SHOP
		RIGHT JOIN dbo.CUSTOMER ON dbo.CUSTOMER.IDENTITYShop = dbo.SHOP.[IDENTITY]
		WHERE";


	if ($tipo == "s") {
		$query = $query . " CAST (dbo.CUSTOMER.DATES AS DATE) >= CAST (GETDATE() - 7 AS DATE) ";
		$titulo = "Semanal";
	} else if ($tipo == "m") {
		$query = $query . " CAST (dbo.CUSTOMER.DATES AS DATE) >= CAST (GETDATE() - 30 AS DATE) ";
		$titulo = "Mensual";
	} else {
		$query = $query . " CAST (dbo.CUSTOMER.DATES AS DATE) >= CAST (GETDATE() AS DATE) ";
	}
	...
	$query = $query . "
		...
		AND dbo.CUSTOMER.WEBLOGIN <> 'Null'
		GROUP BY
			dbo.SHOP.[IDENTITY],
			dbo.SHOP.INTERNALNAME
		ORDER BY
			total DESC";
	...
	echo "<center>";
	echo "<table class='blueTable' width=950px>";
	echo "<thead>";
	...
	echo "<th width='40'><div align='center'><b>Total</b></div></td>";
	echo "</tr>";
	echo "</thead>";
	echo "<tbody>";
